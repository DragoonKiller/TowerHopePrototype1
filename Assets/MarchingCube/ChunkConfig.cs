...
namespace MarchingCube
{
    ...
    [CreateAssetMenu(fileName = "ChunkConfig", menuName = "ScriptableObjects/Chunk Config", order = 1)]
    public class ChunkConfig : ScriptableObject
    {
        ...
        /// Return the chunk id for a world position.
        /// If it is on the edge of multiple chunks, return the upper-right one.
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Vector2Int ChunkBelong(Vector2 worldLoc)
            => new Vector2Int(
                (int)(worldLoc.x / size.x),
                (int)(worldLoc.y / size.y)
            );
        ...
        public List<Vector2Int> ChunksNearby(Vector2 pos)
        {
            ...
            Vector2Int limit = new Vector2Int(
                (maxChunkDistance / size.x).CeilToInt() + 1,
                (maxChunkDistance / size.y).CeilToInt() + 1
            );
